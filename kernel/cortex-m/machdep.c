...
_irq_handler_ (void)
{
	...
	if (h->handler) {
		...
		//для системмного таймера используем такой вариант работы с прерыванием,
		//при котором быстрый обработчик выполняется всегда
		if(irq == ARCH_TIMER_IRQ){
			if ((h->handler) (h->arg) != 0) {
				goto done;
			}
			//поидее для системеого таймера кроме быстрого обработчика ниче не вызываем
			// if (h->lock->master) {
			// 	h->pending = 1;
			// 	goto done;
			// }	
